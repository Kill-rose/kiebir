...
;	メニューボタン非表示
	...
	if(tf.user_setting != 'white'){
		TG.config.alreadyReadTextColor = 'white'; // 一時的に既読テキストの文字色を変更しないようにしています
	}
...
;	未読スキップ-ON
	[button name="unread_on"  fix="true" target="*skip_on"  graphic="&tf.btn_path_off" width="170" height="45" x="570" y="470"]
...
;	テキスト表示速度のサンプルに使用するメッセージレイヤの設定
	[position layer="message1" left="119" top="552" width="1042" height="120" margint="10" marginl="10" page="fore" visible="true" opacity="0"]
...
;	サンプルテキストを表示する
	[emb exp="tf.text_sample"]
		...
		[iscript]
		$(".current_span").css("color","white"); // サンプルテキストのフォントカラーを指定
